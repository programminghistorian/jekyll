...
file. This file will be saved with an `.html`{.filename} extension
rather than a `.txt`{.filename} extension.
...
Save the above program as `write-html.py`{.filename} and execute it. Use
File -\> Open in your chosen text editor to open
`helloworld.html`{.filename} to verify that your program actually
created the file. The content should look like this:

![HTML generated by Python][]

HTML Source Generated by Python Program

Now go to your Firefox browser and choose `File -> New Tab`{.filename},
go to the tab, and choose `File -> Open File`{.filename}. Select
`helloworld.html`{.filename}. You should now be able to see your message
in the browser. Take a moment to think about this: you now have the
ability to write a program which can automatically create a webpage.
There is no reason why you could not write a program to automatically
create a whole website if you wanted to.
...
the code below and save it as `write-html-2.py`{.filename}. When you
execute it, it should create your HTML file and then automatically open
it in a new tab in Firefox. Sweet!

#### Mac Instructions

Mac users will have to specify to the precise location of the
`.html`{.filename} file on their computer. To do this, locate the
`programming-historian`{.filename} folder you created to do these
tutorials, right-click it and select “Get Info”.
