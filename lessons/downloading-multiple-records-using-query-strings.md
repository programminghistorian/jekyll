...
author: Adam Crymble
date: 11-11-2012
reviewers: Fred Gibbs, Luke Bergmann, Fred Gibbs
---
...
``` {.brush: .python; .highlight: .[8,14,37]; .title: .; .notranslate title=""}
...
``` {.brush: .python; .highlight: .[6]; .title: .; .notranslate title=""}
#create URLs for search results pages and save the files
def getSearchResults(query, kwparse, fromYear, fromMonth, toYear, toMonth, entries):
...
``` {.brush: .python; .highlight: .[4,6,9,10,39]; .title: .; .notranslate title=""}
#create URLs for search results pages and save the files
def getSearchResults(query, kwparse, fromYear, fromMonth, toYear, toMonth, entries):
...
``` {.brush: .python; .highlight: .[2,5,25]; .title: .; .notranslate title=""}
def getIndivTrials(query):
...
``` {.brush: .python; .highlight: .[4,45,46]; .title: .; .notranslate title=""}
#create URLs for search results pages and save the files
def getSearchResults(query, kwparse, fromYear, fromMonth, toYear, toMonth, entries):
...
### 3 Responses to “Downloading Multiple Records Using Query Strings” {#comments}

1.  [Thoughts on Public & Digital History by Adam Crymble: Whose
    Lexicon? The Impact of Reporters and Editors on the Old Bailey
    Proceedings][] says:

    [January 17, 2013 at 5:21 pm][]

    [...] of XML files, downloaded from the Old Bailey Online website
    (to learn how to do this check out the Programming Historian 2). I
    decided to focus on the records between the beginning in 1674 and
    the foundation of the Central [...]

    [Reply][]

2.  [Thoughts on Public & Digital History by Adam Crymble: Measuring the
    Diversity of Immigration using the Old Bailey Online 1674-1834][]
    says:

    [January 29, 2013 at 5:01 pm][]

    [...] (OBO) corpus. I've downloaded all of the trial transcripts
    from 1674 to 1834 (find out how on the Programming Historian 2),
    which is about 100,000 trials and 51 million words of text. In the
    last post I looked at the [...]

    [Reply][3]

3.  [Using the Internet Archive and Voyant in my Workflow: Early
    Internet Forums | Ian Milligan][] says:

    [February 9, 2013 at 4:38 pm][]

    [...] in Mathematica to bring these down, although you could adapt
    code and experiment with Python using the Programming Historian 2 to
    do the same. If any of you are using Mathematica, send me an e-mail
    and I can share the notebooks with you. [...]
...
Website
  ...
  [Thoughts on Public & Digital History by Adam Crymble: Whose Lexicon?
  The Impact of Reporters and Editors on the Old Bailey Proceedings]: http://digitalhumanitiesnow.org/2013/01/editors-choice-whose-lexicon-the-impact-of-reporters-and-editors-on-the-old-bailey-proceedings/
  [January 17, 2013 at 5:21 pm]: http://programminghistorian.org/lessons/downloading-multiple-records-using-query-strings#comment-7165
  [Reply]: /lessons/downloading-multiple-records-using-query-strings?replytocom=7165#respond
  [Thoughts on Public & Digital History by Adam Crymble: Measuring the
  Diversity of Immigration using the Old Bailey Online 1674-1834]: http://digitalhumanitiesnow.org/2013/01/editors-choice-measuring-the-diversity-of-immigration-using-the-old-bailey-online-1674-1834/
  [January 29, 2013 at 5:01 pm]: http://programminghistorian.org/lessons/downloading-multiple-records-using-query-strings#comment-7772
  [3]: /lessons/downloading-multiple-records-using-query-strings?replytocom=7772#respond
  [Using the Internet Archive and Voyant in my Workflow: Early Internet
  Forums | Ian Milligan]: http://ianmilligan.ca/2013/02/09/using-the-internet-archive-and-voyant-in-my-workflow-early-internet-forums/
  [February 9, 2013 at 4:38 pm]: http://programminghistorian.org/lessons/downloading-multiple-records-using-query-strings#comment-8214
  [4]: /lessons/downloading-multiple-records-using-query-strings?replytocom=8214#respond
  [Click here to cancel reply.]: /lessons/downloading-multiple-records-using-query-strings#respond
